...
export default function IzordGenerator({role,userId,onUnsavedChange,accessSignal,recovery,onDraft}:{role:string;userId:string;onUnsavedChange:(value:boolean)=>void;accessSignal:AbortSignal;recovery?:GeneratorDraft;onDraft:(draft:GeneratorDraft)=>void}) {
  ...
  function jsonDownload(){if(!accessSignal.aborted&&canWrite&&data)download(new Blob([exportProjectJson(data)],{type:"application/json"}),safeName(data.state.project)+(dirty?"-recuperation":"")+".json");}
