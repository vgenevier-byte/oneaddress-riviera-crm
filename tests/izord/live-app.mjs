...
const dir = mkdtempSync(join(tmpdir(), 'izord-live-app-'));
chmodSync(dir, 0o700);
...
cpSync(join(repo, 'tests/izord/local-network-guard.cjs'), guard);
...
let child;
function sanitized(value) { return String(value).replaceAll(status.ANON_KEY, '[LOCAL PUBLIC KEY]').replace(/eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/g, '[JWT REDACTED]'); }
...
console.log(`Isolated local app manifest: ${manifestPath}\nActual Auth/RLS; Google transport simulated; loopback http://127.0.0.1:3159`);
const exitPromise = launch(['start', '--hostname', '127.0.0.1', '--port', '3159'], serverLog);
manifest.serverPID = child.pid;
let ready = false;
for (let attempt = 0; attempt < 100; attempt++) {
  try {
    const response = await fetch('http://127.0.0.1:3159/api/drive/diagnostic', { redirect: 'error', signal: AbortSignal.timeout(1000) });
...
if (ready) {
  ...
  console.log('READY actual local Next application at http://127.0.0.1:3159');
} else {
  child.kill('SIGTERM');
